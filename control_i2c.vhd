...
-- Tipo de operacion (lectura o escritura)
  signal nWR_op: std_logic; --tipo de operacion
  signal nWR:    std_logic; --tipo de operacion que se ejecuta al momento
...
begin
  ...
  -- Control del generador de SCL:
                       -- ******************************: ejercicio 6
  ena_SCL <= '1' when estado /= libre and estado /= stop else 
             '0';                                            -- Se habilita tras ini
  ...
  reset_SDA <= ini         when estado = libre           else     -- Condicion de start  (ini cuando estado = libre)
               ...
               '0';
                       ...
                       -- ******************************: ejercicio 8
  ACK_lectura <= (nWR and (not last_byte)) when estado = ACK else 
                  '0';                                            -- en una operacion de lectura deben asentirse todos los bytes leidos
                                                                  -- menos el ultimo

                       -- ******************************: ejercicio 7
  preset_SDA <= ena_stop_i2c when estado = stop else 
                '0';                                              -- Condición de stop(segnalizacion de gen_SCL cuando estado = stop)
                       ...
                       -- ******************************: ejercicio 5
  desplaza_reg_out_SDA <= ena_out_SDA when estado = tx_byte else 
                          '0';                                    -- Se desplaza el dato cuando lo indica la segnal de gen_SCL                         
