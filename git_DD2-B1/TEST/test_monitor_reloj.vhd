 ...
 -- MONITOR 3
	-- Comprueba:
		...
		-- 2) Comprueba el funcionamiento normal en MODO 12h (info = 0 y modo = 0):
			-- 2.1) Si el reloj acaba de dar la vuelta (la hora actual es 00:00:00), comprueba que la señal AM_PM hay acambiado respecto al segundo anterior (AM_PM /= AM_PM_T1).
			-- 2.2) Si la hora NO es 00:00:00, comprueba que la señal AM_PM se mantenga igual que en el segundo anterior (AM_PM = AM_PM_T1).
  ...
  begin
    if nRst'event and nRst = '0' then
      ...
      if ena_cmd_T1 = '1' and cmd_tecla_T1 = X"B" and info_T1 /= 0 then
        if info_T1 = 1 then
          assert info = 2
          report "Error al cambiar el campo en edicion de minutos a horas"  
          severity error;

        else
          assert info = 1
          report "Error al cambiar el campo en edicion de horas a minutos" 
          severity error;
    ...
    if nRst'event and nRst = '0' then
      ...
      if ena_cmd = '1' and cmd_tecla < x"A" and cmd_tecla_T1 < x"A" and
        ...
        if info = 1 then
          ...
          report "Monitor 10: Error al programar los minutos mediante pulsacion directa"
          severity error;

        -- Se modifican las horas
        else
          assert horas(7 downto 0) = cmd_tecla_T1&cmd_tecla
          report "Monitor 10: Error al programar las horas mediante pulsacion directa"
          severity error;
